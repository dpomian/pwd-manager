...
class User(db.Model):
    ...
    # Legacy plaintext DEK.  Nullable because new wrapped accounts do not
    # store a plaintext DEK; kept for one release as a fallback.
    encryption_key = db.Column(db.String(255), nullable=True)
    # Phase 2 key-wrapping columns
    kdf_salt = db.Column(db.String(64), nullable=True)
    kdf_iterations = db.Column(db.Integer, nullable=True)
    wrapped_dek = db.Column(db.String(255), nullable=True)
    key_version = db.Column(db.Integer, default=0, nullable=False)
    ...
    def __init__(self, username, password=None):
        ...
        if password:
            ...
            # For testing purposes, set a dummy password and a random
            # legacy encryption key so the instance is usable without KDF.
    ...
    def set_password(self, password):
        """Hash the password and generate (or wrap) the data-encryption key."""
        ...
        if is_enabled("ENABLE_KEY_WRAPPING"):
            ...
            self.key_version = 1
            # New accounts store no plaintext DEK; the legacy column is kept
            # for fallback during the one-release migration window.
            self.encryption_key = None
        else:
            # Legacy: the raw DEK is stored in the DB as a base64 string.
            self.encryption_key = b64encode(os.urandom(32)).decode("utf-8")
            self.key_version = 0
            self.wrapped_dek = None
            self.kdf_salt = None
    ...
    def get_dek(self, password: str) -> str:
        ...
        For wrapped accounts (key_version=1) this derives the KEK and
        decrypts the stored wrapped key.  For legacy accounts (key_version=0)
        it returns the stored plaintext DEK.
        """
        from cryptography.fernet import Fernet
        ...
        if self.key_version == 0:
            if not self.encryption_key:
                raise ValueError("No legacy encryption key available")
